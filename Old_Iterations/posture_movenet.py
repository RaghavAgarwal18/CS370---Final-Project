...
# How much neck_y ratio must change from baseline to flag bad posture
# neck_y is normalized by shoulder width so camera distance doesnt matter
# From your data: upright ~1.2-1.4, forward ~1.6-1.9
# Deviation of 0.25 gives good buffer without being too sensitive
NECK_FORWARD_THRESH = 0.25
...
try:
    while True:
        ...
        if shoulders_ok and ears_ok:
            ...
            # neck_y normalized by shoulder width
            # = vertical distance ear-to-shoulder as fraction of shoulder width
            # upright: ears well above shoulders = large value
            # leaning forward: ears come forward/down relative to shoulders = smaller value
            ...
            # ===== DETECTION =====
                ...
                # neck_diff negative = head moved forward/down from baseline
                # neck_diff positive = head moved further up/back
                # we only care about forward lean so check neck_diff < -threshold
                ...
                # Redraw spine line with correct color
                cv2.line(frame, s_mid, e_mid, color, 3)
