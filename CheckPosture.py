...
class PostureTracker:
    def __init__(self):
        ...
        self.slouch_buffer = 0
        self.slouch_threshold_frames = 15 # Must slouch for ~15 processed frames
    ...
    def _find_face_smart(self, gray, prev_face):
        if prev_face is not None:
            x, y, w, h = prev_face
            # Expand the search area by 20%
            ...
            faces = self.face_cascade.detectMultiScale(roi_gray, 1.2, 5)
            if len(faces) > 0:
                fx, fy, fw, fh = max(faces, key=lambda r: r[2]*r[3])
                return (roi_x + fx, roi_y + fy, fw, fh)

        # Fallback to full screen if ROI fails
        small_gray = cv2.resize(gray, (0,0), fx=self.detect_scale, fy=self.detect_scale)
        faces = self.face_cascade.detectMultiScale(small_gray, 1.2, 5)
    ...
    def _find_eye_angle(self, gray, face):
        """Calculate the angle between eyes to detect head tilt."""
        try:
            x, y, w, h = face
            # Look for eyes only in the top half of the face box
            roi_gray = gray[y:y + int(h / 1.8), x:x + w]
            eyes = self.eye_cascade.detectMultiScale(roi_gray, 1.1, 5, minSize=(25, 25))
            ...
            if len(eyes) >= 2:
                ...
                # Center points: (x + width/2, y + height/2)
                p1 = (e1[0] + e1[2] / 2, e1[1] + e1[3] / 2)
                p2 = (e2[0] + e2[2] / 2, e2[1] + e2[3] / 2)
                
                # dy = y2 - y1, dx = x2 - x1
                dy = p2[1] - p1[1]
                dx = p2[0] - p1[0] # Fixed: changed p2[2] to p2[0]
